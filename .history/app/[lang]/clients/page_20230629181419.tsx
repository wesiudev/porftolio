...
export default async function ClientsPage({
  ...
  return (
    <>
      <section className="flex flex-col lg:flex-row min-h-screen lg:h-screen items-center font-sans relative overflow-x-hidden z-[1500]">
        <div className="bg-gradient-to-br linear-gradient flex w-full lg:w-1/2 h-[90vh] lg:h-screen relative pt-12 lg:pt-0">
          <div className="lg:w-full lg:mx-6 xl:mx-12 flex items-center justify-center text-center">
            <div className="lg:bg-white text-white lg:text-black flex flex-col items-center justify-center py-12 rounded-lg px-6 lg:relative">
              <Link
                href={`/${params.lang}`}
                className="absolute left-12 top-12 lg:left-0 lg:-top-16 flex flex-row text-white text-xl items-center sm:text-2xl"
              >
                <FaArrowLeft /> <span>{dictionary["Navigation"].goBack}</span>
              </Link>
              <Image
                unoptimized
                ...
                className="rounded-lg w-3/4 sm:w-1/2 scale-100 duration-200"
              />
