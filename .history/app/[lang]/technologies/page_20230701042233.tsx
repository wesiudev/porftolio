        ...
        <div className="w-3/4 grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mx-auto transition-all duration-100 relative overflow-hidden">
          ...
          {dictionary["Technologies"].map((tech: any, idx: number) => (
            <div
              ...
              key={idx}
              className="relative p-3 bg-white flex flex-col duration-75 group rounded-md shadow-lg hover:shadow-xl hover:scale-105 hover:-translate-y-3 cursor-pointer"
            >
              <h1 className="text-2xl pb-2 cursor-default absolute left-4 top-4">
                {tech.name}
              </h1>
              <div className="flex items-center justify-center py-6 bg-gradient-to-r from-gray-400 via-white to-gray-400 h-[25vh] rounded-md">
                <Image
                  unoptimized
                  src={tech.image}
