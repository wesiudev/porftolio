          ...
          {dictionary.CustomerHook.templates.map(
            (template: any, idx: number) => (
              <div className="bg-white h-max aspect-square rounded-md overflow-hidden relative group">
                ...
                <span className="group-hover:translate-y-[50%] text-white absolute z-50 left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] text-2xl w-max cursor-default select-none">
                  {template.name}
                </span>
              </div>
            )
