    ...
    <section className={`min-h-screen min-w-screen relative flex-col`}>
      ...
      {hoveredText.itemName !== "" && (
        <div className="fixed top-0 lg:bottom-6 left-0 w-full z-[5000] ">
          <div className="lg:hidden flex flex-col w-full h-screen lg:w-3/4 mx-auto bg-white rounded-t-lg lg:rounded-lg lg:p-6 sm:text-xl lg:text-2xl">
            ...
            <div className="h-full flex items-center flex-row justify-center">
              {dictionary.CustomerHook.templates.map(
                (template: any, idx: number) => (
                  <div
                    ...
                    key={idx}
                    className="bg-white h-full aspect-square rounded-md overflow-hidden relative group"
                  >
                    <img
                      className="filter brightness-[0.62] h-full"
                      src={template.img}
