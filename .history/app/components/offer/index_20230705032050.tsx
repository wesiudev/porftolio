...
export default function CustomerHook({ dictionary }: { dictionary: any }) {
  ...
  return (
    <section className={`min-h-screen min-w-screen relative flex-col`}>
      <div
        className={`linear-gradient px-3 w-full text-center lg:px-0 lg:w-3/4 mx-auto h-max text-white flex flex-col justify-center items-center rounded-lg pb-3 sm:pb-6 lg:pb-12`}
      >
        <h1 className="text-xl sm:text-2xl lg:text-4xl py-3">
          {dictionary.CustomerHook.h1}
        </h1>
        <h2 className="text-lg sm:text-2xl  px-3 sm:px-12 pb-3 sm:pb-6 lg:pb-12">
          {dictionary.CustomerHook.h2}
        </h2>
      </div>
      {hoveredText.itemName !== "" && (
        <div className="fixed top-0 sm:top-3 lg:top-6 left-0 sm:h-max w-full z-[5000]">
          <div
            onMouseOver={() =>
              width > 1024 &&
              setHoveredText({
            ...
            className=" flex flex-col w-full h-screen sm:h-max lg:w-3/4 mx-auto bg-white rounded-t-lg lg:rounded-lg lg:p-6 sm:text-xl lg:text-2xl overflow-scroll pb-12 sm:pb-0 lg:overflow-hidden"
          >
            ...
            <span className="text-xl font-bold pt-6 sm:py-3 lg:pt-0 p-3 lg:text-3xl select-none">
              {hoveredText.itemName}
            </span>
            <span className="italic py-3 sm:py-0 p-3 select-none">
