  ...
  return (
    <section className={`min-h-screen min-w-screen relative flex-col`}>
      ...
      {hoveredText.itemName !== "" && (
        <div className="fixed top-0 lg:bottom-6 left-0 w-full h-screen z-[5000] ">
          <div className="flex flex-col w-full  lg:w-3/4 mx-auto h-full bg-white rounded-t-lg lg:rounded-lg lg:p-6 sm:text-xl lg:text-2xl">
            <img src={hoveredText.img} alt="Mobile website description" />
            <span className="font-bold p-3">{hoveredText.itemName}</span>
            <span className="italic p-3">{hoveredText.itemDescription}</span>
          </div>
        </div>
      )}
          ...
          {dictionary.CustomerHook.templates.map(
            (template: any, idx: number) => (
              ...
              >
                ...
                />
                <span className="group-hover:lg:scale-110 duration-150 text-lg text-white absolute z-50 left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] lg:text-2xl w-max cursor-default select-none ">
                  {template.name}
                </span>
                <button
