...
export default function CustomerHook({ dictionary }: { dictionary: any }) {
  ...
  return (
    <section className={`min-h-screen min-w-screen relative flex-col`}>
      ...
      {hovered && (
        <div className="fixed bottom-6 left-0 w-full z-[1500] ">
          <div className="w-full px-3 lg:w-3/4 mx-auto bg-white rounded-lg lg:p-6 text-center lg:text-left sm:text-xl lg:text-2xl">
            {hovered}
          </div>
        </div>
      )}
      <div className="w-full h-max flex flex-col mt-6 px-3 lg:px-0 lg:w-3/4 mx-auto rounded-lg z-50">
        <div className="grid grid-cols-2 lg:grid-cols-4 h-max gap-6">
          {dictionary.CustomerHook.templates.map(
            (template: any, idx: number) => (
              <div className="bg-white h-max aspect-square rounded-md overflow-hidden relative group">
                <Canvas
                  onPointerOver={() => setHovered({itemName:template.name, template.description})}
                ...
                <span className="text-lg  text-white absolute z-50 left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] lg:text-2xl w-max cursor-default select-none ">
                  {template.name}
                </span>
              </div>
            )
