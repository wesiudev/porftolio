...
export default function CustomerHook({ dictionary }: { dictionary: any }) {
  ...
  return (
    <section
      className={`min-h-screen min-w-screen relative flex-col overflow-x-hidden`}
    >
      <div
      ...
      {hoveredText.itemName !== "" && (
        <div className="fixed top-0 lg:bottom-6 left-0 w-full z-[5000] ">
          <div className="flex flex-col w-full h-screen lg:w-3/4 mx-auto bg-white rounded-t-lg lg:rounded-lg lg:p-6 sm:text-xl lg:text-2xl overflow-y-scroll  pb-12 lg:pb-0">
            ...
            <div className="w-full h-full grid grid-cols-2 lg:grid-cols-6 gap-3 px-3">
            ...
            (template: any, idx: number) => (
              ...
              >
                <img
                  ...
                  alt="Website type image"
                />
                <span className="text-sm group-hover:lg:scale-110 duration-150 sm:text-lg text-white absolute z-50 left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] w-max cursor-default select-none ">
                  {template.name}
                </span>
                <button
