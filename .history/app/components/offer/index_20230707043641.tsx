...
export default function CustomerHook({ dictionary }: { dictionary: any }) {
  ...
  const services =
    chosenProduct && chosenProduct.services.map((service: any) => service.id);
  const chosenProductServiceTime = dictionary.CustomerHook.toolServices.filter(
    (service: any) => services.includes(service.id)
  );
