        ...
        <div className="grid grid-cols-3 h-max gap-6">
          {dictionary.CustomerHook.templates.map(
            (template: any, idx: number) => (
              <div className="bg-white h-max rounded-md relative">
                <img
                  className="rounded-md filter brightness-[0.62] drop-shadow-md shadow-black"
                  src={template.img}
                  alt="template"
                />
                <span className="text-white absolute left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] text-2xl w-max cursor-default select-none">
                  {template.name}
                </span>
              </div>
            )
