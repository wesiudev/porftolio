          ...
          {dictionary.CustomerHook.templates.map(
            (template: any, idx: number) => (
              <div className="bg-white h-max rounded-md relative">
                <img
                  className="rounded-md filter brightness-75 drop-shadow-md shadow-black"
                  src={template.img}
                  alt="template"
                />
                <span className="text-white absolute left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] text-2xl w-max drop-shadow-md shadow-black">
                  {template.name}
                </span>
              </div>
            )
