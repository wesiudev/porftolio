...
export default function CustomerHook({ dictionary }: { dictionary: any }) {
  ...
  function getExpectedTime() {
    const services = chosenProduct.services.map((service: any) => service.id);
    const chosenProductServiceTime =
      dictionary.CustomerHook.toolServices.filter((service: any) =>
        services.includes(service.id)
      );
    const timeArray = chosenProductServiceTime.map(
      (service: any) => (service.durationLogic, service.duration)
    );
    const calculatedTime = timeArray.reduce(
      (acc: any, curr: any) => acc + sumNumbersFromString(curr),
      0
    );
    setExpectedTime(calculatedTime);
  }
  return (
    <section className={`min-h-screen w-full relative flex-col `}>
      ...
      <div
        ...
        } w-full  bg-gradient-to-br from-slate-600 via-gray-500 to-slate-600 mt-60 lg:px-6 relative `}
      >
        <div className="-top-48" id="tool" />
        {chosenProduct.length !== 0 ? (
          <div className="w-full h-full px-3 lg:px-12 lg:w-full mx-auto">
            ...
            <div className="flex flex-col lg:mt-12 w-full cursor-default select-none">
              ...
              <div className="text-xl sm:text-2xl lg:text-4xl xl:text-5xl text-white grid md:grid-cols-2 xl:grid-cols-3 gap-3 mt-12">
                {chosenProduct.services?.map((service: any, idx: number) => (
                  <div
                    data-aos="fade-up"
                    data-aos-delay={100}
                    key={idx}
                    className={`flex flex-col p-6 sm:p-3 sm:py-6 xl:p-6 bg-slate-800 drop-shadow-xl shadow-black rounded-lg `}
                  >
                    <div
                    ...
                    <div className="flex flex-col h-full justify-end ">
                      ...
                      <div className={`flex flex-col lg:flex-row w-full`}>
                        ...
                        >
                          ...
                          <FaClock className="w-12 h-12 mx-6 xl:mx-8" />
